...
const apiUrl = 'https://dandan-myflix.herokuapp.com/'
@Injectable({
  providedIn: 'root'
})
export class UserRegistrationSercvice {
  ...
  addFavorite(movieID: any): Observable<any> {
    ...
    return this.http.post(apiUrl + `${username}/movies/${movieID}`, {}, {
      headers: new HttpHeaders({ Authorization: 'Bearer ' + token })
    }).pipe(
      map(this.extractResponseData),
      catchError(this.handleError)
  ...
  deleteFavorite(username: any, movieID: any): Observable<any> {
    const token = localStorage.getItem('token');
    return this.http.delete(apiUrl + `${username}/movies/${movieID}`, {
      headers: new HttpHeaders({ Authorization: 'Bearer ' + token })
    }).pipe(
      map(this.extractResponseData),
      catchError(this.handleError)
